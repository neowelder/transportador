...
/**
 * Base static class for performing query and update operations on the 'usertype' table.
 *
 * 
 *
...
abstract class BaseUsertypePeer {
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseUsertypePeer', $criteria, $con);
		}
	...
	/**
	 * Method to invalidate the instance pool of all tables related to usertype
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		$cls = UsertypePeer::getOMClass(false);
	...
	{
		return $withPrefix ? UsertypePeer::CLASS_DEFAULT : UsertypePeer::OM_CLASS;
	}
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseUsertypePeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseUsertypePeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseUsertypePeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseUsertypePeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseUsertypePeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseUsertypePeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseUsertypePeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseUsertypePeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the usertype table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseUsertypePeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
